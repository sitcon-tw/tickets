	...
	useEffect(() => {
		const loadRanking = async () => {
			try {
				...
				setCurrentEventId(foundEvent.id);

				// Check if user has registration for this event
				try {
					const registrations = await registrationsAPI.getAll();
					const eventRegistration = registrations.data.find(reg => reg.event?.id === foundEvent.id);
					if (!eventRegistration) {
						// User doesn't have registration, redirect to home
						router.push("/");
						return;
					}
				} catch {
					// User not logged in, that's OK for viewing ranking
				}

				// Fetch ranking data
				const rankingResponse = await referralsAPI.getRanking(foundEvent.id, 50);
				setRankingData(rankingResponse.data);
			} catch (error) {
				console.error("Failed to load referral ranking:", error);
				setError(true);
			} finally {
				setLoading(false);
			}
