    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2">
          ...
          {ticket.hidden && (
            <span className="py-0.5 px-2 text-xs font-bold text-gray-100 dark:text-gray-200 bg-gray-600 dark:bg-gray-700 rounded border border-gray-500 dark:border-gray-600">
              {actions.t.hidden}
            </span>
          )}
        </div>
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-2 flex-wrap">
          ...
          <Button
            size="sm"
            onClick={() => actions.onLinkBuilder(ticket)}
          >
          ...
          <Button
            variant="destructive"
            size="sm"
            onClick={() => actions.onDelete(ticket.id)}
          >
            {actions.t.delete}
          </Button>
        </div>
