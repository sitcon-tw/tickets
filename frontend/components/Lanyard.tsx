...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false, name }: BandProps) {
	...
	const { nodes, materials } = useGLTF(cardGLB) as ObjectMap & CardGLTF;
	...
	useFrame((state, delta) => {
		...
		if (fixed.current && j1.current && j2.current && j3.current && card.current && band.current) {
			const [j1Lerped, j2Lerped] = [j1.current, j2.current].map(body => {
				body.lerped ??= new THREE.Vector3().copy(body.translation());
				const clampedDistance = Math.max(0.1, Math.min(1, body.lerped.distanceTo(body.translation())));
				return body.lerped.lerp(body.translation(), delta * (minSpeed + clampedDistance * (maxSpeed - minSpeed)));
			});
