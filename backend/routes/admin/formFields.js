...
export default async function adminFormFieldsRoutes(fastify, options) {
  ...
  async (request, reply) => {
    try {
      const {
        name, label, type, description, required, order,
        options, validation, placeholder, helpText
      } = request.body;
  ...
  // 更新表單欄位
  fastify.put('/form-fields/:fieldId',
  { schema: {
    description: '更新表單欄位',
    tags: ['admin-form-fields'],
  }},
  async (request, reply) => {
    try {
      ...
      const {
        label, description, required, order,
        options, validation, placeholder, helpText
      } = request.body;
  ...
  // 刪除表單欄位
  fastify.delete('/form-fields/:fieldId',
  { schema: {
    description: '刪除表單欄位',
    tags: ['admin-form-fields'],
  }},
  ...
  // 設定欄位與票種的顯示/必填關係
  fastify.put('/form-fields/:fieldId/ticket-binding',
  { schema: {
