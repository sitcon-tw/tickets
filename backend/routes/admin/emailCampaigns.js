...
export default async function adminEmailCampaignsRoutes(fastify, options) {
  ...
  async (request, reply) => {
    try {
      const {
        name, subject, htmlContent, textContent, recipients,
        scheduleTime, attachments
      } = request.body;
  ...
  // 獲取郵件發送狀態
  fastify.get('/email-campaigns/:campaignId/status',
  { schema: {
  ...
  // 預覽郵件內容
  fastify.post('/email-campaigns/:campaignId/preview',
  { schema: {
    description: '預覽郵件內容',
    tags: ['admin-email-campaigns'],
  }},
  ...
  // 取消郵件發送任務
  fastify.delete('/email-campaigns/:campaignId',
  { schema: {
